  ...
  constructor(private _guestService: GuestService, private _clienteService: ClienteService){
    ...
    this._clienteService.obtener_config_publico().subscribe((response) => {
      ...
      response.data.categorias.forEach(element => {
        if(element.titulo == 'Sneakers'){
          this.categorias.push({
            titulo: element.titulo,
            portada: 'assets/img/ecommerce/home/categories/002.jpg'
          });
        } else if(element.titulo == 'Accesorios'){
          this.categorias.push({
            titulo: element.titulo,
            portada: 'assets/img/ecommerce/home/categories/003.jpg'
          });
        } else if(element.titulo == 'Streetwear'){
          this.categorias.push({
            titulo: element.titulo,
            portada: 'assets/img/ecommerce/home/categories/001.jpg'
          });
        } else if(element.titulo == 'Decoración'){
          this.categorias.push({
            titulo: element.titulo,
            portada: 'assets/img/ecommerce/home/categories/004.jpg'
          });
        }
      });
